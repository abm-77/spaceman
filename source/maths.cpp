...
v2 V2RotateAboutV2 (v2 point, v2 center, f32 angle) {
    v2 dist = V2MinusV2(point, center);

    f32 dx =  (dist.x * Cos(angle)) - (dist.y * Sin(angle));
    f32 dy =  (dist.y * Cos(angle)) + (dist.x * Sin(angle));

    v2 c = {
        center.x + dx,
        center.y + dy // minus comes from up being negative
    };
